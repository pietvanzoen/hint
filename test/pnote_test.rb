...
class TestNote < Test::Unit::TestCase
  ...
  def test_initialize
    ...
    assert_equal(Time.now.to_s, @note.created_at.to_s)
    assert_equal(Time.now.to_s, @note.updated_at.to_s)
  ...
  def test_archive
    ...
    @note.archive!
    assert_equal(Time.now.to_s, @note.archived_at.to_s)
...
class TestNotesRepo < Test::Unit::TestCase
  def setup
    ...
    @note_jan = Note.new("id" => "1", "book" => "git", "content" => "jan", "created_at" => Time.parse("2018-01-01").to_s)
    @note_feb = Note.new("id" => "2", "book" => "git", "content" => "feb", "created_at" => Time.parse("2018-02-01").to_s)
    @note_mar = Note.new("id" => "3", "book" => "bash", "content" => "mar", "created_at" => Time.parse("2018-03-01").to_s)
    @note_archived = Note.new("id" => "4", "book" => "bash", "content" => "archived", "created_at" => Time.parse("2018-03-01").to_s, "archived_at" => Time.now.to_s)
  ...
  def test_archive!
    ...
    assert_equal(Time.now.to_s, archived_note["archived_at"])
  end
end
